<?xml version="1.0" encoding="UTF-8"?>
<nhi:FeatureCollection xmlns:nhi="http://www.nhi.nu/gml" xmlns:gml="http://www.opengis.net/gml" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" hydamoVersion="13" xsi:schemaLocation="http://www.nhi.nu/gml ../nhiFeatureTypes.xsd">
	<gml:boundedBy>
		<gml:null>unknown</gml:null>
	</gml:boundedBy>
		<!-- Koppeling naar Doorstroomopening -->
	<gml:featureMember>
		 <nhi:Afsluitmiddel>
			<nhi:code>asm1</nhi:code>
			<nhi:statusObject>1</nhi:statusObject>
			<nhi:CREATED_DATE>2001-12-17T09:30:47Z</nhi:CREATED_DATE>
			<nhi:LAST_EDITED_DATE>2001-12-17T09:30:47Z</nhi:LAST_EDITED_DATE>
			<nhi:administratiefgebied>1</nhi:administratiefgebied>
			<nhi:naam>naam</nhi:naam>
			<nhi:hyperlink>http://ergens.naar.toe</nhi:hyperlink>
			<nhi:doorstroomopeningID>dso1</nhi:doorstroomopeningID>
			<nhi:soortAfsluitmiddel>1</nhi:soortAfsluitmiddel>
			<nhi:soortRegelbaarheid>1</nhi:soortRegelbaarheid>
			<nhi:overlaatOnderlaat>1</nhi:overlaatOnderlaat>
			<nhi:breedte>1.0</nhi:breedte>
			<nhi:hoogte>1.0</nhi:hoogte>
			<nhi:minimaleHoogteBovenkant>1.0</nhi:minimaleHoogteBovenkant>
			<nhi:maximaleHoogteBovenkant>1.0</nhi:maximaleHoogteBovenkant>
			<nhi:afvoerCoefficient>1.0</nhi:afvoerCoefficient>
			<gml:pointProperty>
				<gml:Point srsName="EPSG:28992"><gml:coordinates>148838.172,424868.313</gml:coordinates></gml:Point>
			</gml:pointProperty>
		</nhi:Afsluitmiddel>
	</gml:featureMember>	
	<!-- Koppeling naar Duikersifonhevel -->	
	<gml:featureMember>
		 <nhi:Afsluitmiddel>
			<nhi:code>asm2</nhi:code>
			<nhi:statusObject>1</nhi:statusObject>
			<nhi:CREATED_DATE>2001-12-17T09:30:47Z</nhi:CREATED_DATE>
			<nhi:LAST_EDITED_DATE>2001-12-17T09:30:47Z</nhi:LAST_EDITED_DATE>
			<nhi:administratiefgebied>1</nhi:administratiefgebied>
			<nhi:naam>naam</nhi:naam>
			<nhi:hyperlink>http://ergens.naar.toe</nhi:hyperlink>
			<nhi:duikersifonhevelID>dsh1</nhi:duikersifonhevelID>
			<nhi:soortAfsluitmiddel>1</nhi:soortAfsluitmiddel>
			<nhi:soortRegelbaarheid>1</nhi:soortRegelbaarheid>
			<nhi:overlaatOnderlaat>1</nhi:overlaatOnderlaat>
			<nhi:breedte>1.0</nhi:breedte>
			<nhi:hoogte>1.0</nhi:hoogte>
			<nhi:minimaleHoogteBovenkant>1.0</nhi:minimaleHoogteBovenkant>
			<nhi:maximaleHoogteBovenkant>1.0</nhi:maximaleHoogteBovenkant>
			<nhi:afvoerCoefficient>1.0</nhi:afvoerCoefficient>
			<gml:pointProperty>
				<gml:Point srsName="EPSG:28992"><gml:coordinates>148838.172,424868.313</gml:coordinates></gml:Point>
			</gml:pointProperty>
		</nhi:Afsluitmiddel>
	</gml:featureMember>	
	<!-- Koppeling naar Gemaal -->
	<gml:featureMember>
		 <nhi:Afsluitmiddel>
			<nhi:code>asm3</nhi:code>
			<nhi:statusObject>1</nhi:statusObject>
			<nhi:CREATED_DATE>2001-12-17T09:30:47Z</nhi:CREATED_DATE>
			<nhi:LAST_EDITED_DATE>2001-12-17T09:30:47Z</nhi:LAST_EDITED_DATE>
			<nhi:administratiefgebied>1</nhi:administratiefgebied>
			<nhi:naam>naam</nhi:naam>
			<nhi:hyperlink>http://ergens.naar.toe</nhi:hyperlink>
			<nhi:gemaalID>g1</nhi:gemaalID>
			<nhi:soortAfsluitmiddel>1</nhi:soortAfsluitmiddel>
			<nhi:soortRegelbaarheid>1</nhi:soortRegelbaarheid>
			<nhi:overlaatOnderlaat>1</nhi:overlaatOnderlaat>
			<nhi:breedte>1.0</nhi:breedte>
			<nhi:hoogte>1.0</nhi:hoogte>
			<nhi:minimaleHoogteBovenkant>1.0</nhi:minimaleHoogteBovenkant>
			<nhi:maximaleHoogteBovenkant>1.0</nhi:maximaleHoogteBovenkant>
			<nhi:afvoerCoefficient>1.0</nhi:afvoerCoefficient>
			<gml:pointProperty>
				<gml:Point srsName="EPSG:28992"><gml:coordinates>148838.172,424868.313</gml:coordinates></gml:Point>
			</gml:pointProperty>
		</nhi:Afsluitmiddel>
	</gml:featureMember>		
	<!-- Koppeling naar Aquqduct -->
	<gml:featureMember>
		 <nhi:Afsluitmiddel>
			<nhi:code>asm4</nhi:code>
			<nhi:statusObject>1</nhi:statusObject>
			<nhi:CREATED_DATE>2001-12-17T09:30:47Z</nhi:CREATED_DATE>
			<nhi:LAST_EDITED_DATE>2001-12-17T09:30:47Z</nhi:LAST_EDITED_DATE>
			<nhi:administratiefgebied>1</nhi:administratiefgebied>
			<nhi:naam>naam</nhi:naam>
			<nhi:hyperlink>http://ergens.naar.toe</nhi:hyperlink>
			<nhi:aquaductID>a1</nhi:aquaductID>
			<nhi:soortAfsluitmiddel>1</nhi:soortAfsluitmiddel>
			<nhi:soortRegelbaarheid>1</nhi:soortRegelbaarheid>
			<nhi:overlaatOnderlaat>1</nhi:overlaatOnderlaat>
			<nhi:breedte>1.0</nhi:breedte>
			<nhi:hoogte>1.0</nhi:hoogte>
			<nhi:minimaleHoogteBovenkant>1.0</nhi:minimaleHoogteBovenkant>
			<nhi:maximaleHoogteBovenkant>1.0</nhi:maximaleHoogteBovenkant>
			<nhi:afvoerCoefficient>1.0</nhi:afvoerCoefficient>
			<gml:pointProperty>
				<gml:Point srsName="EPSG:28992"><gml:coordinates>148838.172,424868.313</gml:coordinates></gml:Point>
			</gml:pointProperty>
		</nhi:Afsluitmiddel>
	</gml:featureMember>		
</nhi:FeatureCollection>
