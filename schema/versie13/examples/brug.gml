<?xml version="1.0" encoding="UTF-8"?>
<nhi:FeatureCollection xmlns:nhi="http://www.nhi.nu/gml" xmlns:gml="http://www.opengis.net/gml" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" nhiVersion="13" xsi:schemaLocation="http://www.nhi.nu/gml ../nhiFeatureTypes.xsd">
	<gml:boundedBy>
		<gml:null>unknown</gml:null>
	</gml:boundedBy>
	<gml:featureMember>
		 <nhi:brug>
			<nhi:code>b1</nhi:code>
			<nhi:statusObject>1</nhi:statusObject>
			<nhi:CREATED_DATE>2001-12-17T09:30:47Z</nhi:CREATED_DATE>
			<nhi:LAST_EDITED_DATE>2001-12-17T09:30:47Z</nhi:LAST_EDITED_DATE>
			<nhi:administratiefgebied>1</nhi:administratiefgebied>			
			<nhi:naam>naam</nhi:naam>
			<nhi:hyperlink>http://ergen.naar.toe</nhi:hyperlink>
			<nhi:hoogteOnderzijde>1.0</nhi:hoogteOnderzijde>
			<nhi:hoogteBovenzijde>1.0</nhi:hoogteBovenzijde>
			<nhi:lengte>1.0</nhi:lengte>
			<nhi:typeRuwheid>1</nhi:typeRuwheid>
			<nhi:ruwheid>1.0</nhi:ruwheid>
			<nhi:intreeverlies>1.0</nhi:intreeverlies>
			<nhi:uittreeverlies>1.0</nhi:uittreeverlies>
			<gml:pointProperty>
				<gml:Point srsName="EPSG:28992"><gml:coordinates>148838.172,424868.313</gml:coordinates></gml:Point>
			</gml:pointProperty>
		</nhi:brug>
	</gml:featureMember>	
	<gml:featureMember>
		 <nhi:brug>
			<nhi:code>b2</nhi:code>
			<nhi:statusObject>1</nhi:statusObject>
			<nhi:CREATED_DATE>2001-12-17T09:30:47Z</nhi:CREATED_DATE>
			<nhi:LAST_EDITED_DATE>2001-12-17T09:30:47Z</nhi:LAST_EDITED_DATE>
			<nhi:administratiefgebied>1</nhi:administratiefgebied>			
			<nhi:naam>naam</nhi:naam>
			<nhi:hyperlink>http://ergen.naar.toe</nhi:hyperlink>
			<nhi:hoogteOnderzijde>1.0</nhi:hoogteOnderzijde>
			<nhi:hoogteBovenzijde>1.0</nhi:hoogteBovenzijde>
			<nhi:lengte>1.0</nhi:lengte>
			<nhi:typeRuwheid>1</nhi:typeRuwheid>
			<nhi:ruwheid>1.0</nhi:ruwheid>
			<nhi:intreeverlies>1.0</nhi:intreeverlies>
			<nhi:uittreeverlies>1.0</nhi:uittreeverlies>
			<gml:pointProperty>
				<gml:Point srsName="EPSG:28992"><gml:coordinates>148838.172,424868.313</gml:coordinates></gml:Point>
			</gml:pointProperty>
		</nhi:brug>
	</gml:featureMember>	
</nhi:FeatureCollection>
